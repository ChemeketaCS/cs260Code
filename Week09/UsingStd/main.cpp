...
int main() {
    ...
    // // Does not work...
    // std::unordered_set<Point> s0;

    //Use PointHasher for hashing, == for equality comparison
    std::unordered_set<Point, PointHasher> s1;
    s1.insert(p1);
    s1.insert(p2);
    ...
    cout << std::count(s1.begin(), s1.end(), p1)  << endl;
    cout << std::count(s1.begin(), s1.end(), p2) << endl;
    cout << std::count(s1.begin(), s1.end(), p3) << endl;
}
