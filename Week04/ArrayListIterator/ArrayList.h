...
public:
    ...
    /**
     * @brief Returns an ArayList iterator pointing to first element in list
     */
    ArrayListIterator<T> begin() {
        //list equivelent to &list[0]
        return ArrayListIterator<T>(list);
    }

    /**
     * @brief Returns an ArayList iterator pointing to memory location after last item in list
     */
    ArrayListIterator<T> end() {
        //list is base address
        //+length increments by length units of size required by T
        return ArrayListIterator<T>(list+length);
    }

    ///------------------End new Iterator Functions-------------------
